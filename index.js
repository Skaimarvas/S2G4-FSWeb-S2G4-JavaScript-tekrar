...
console.log("Örnek Görev : Karenin alanı:", KareninAlani(5));
...
function CemberinCevresi(cemberYariCap) {
  const cemberCevre = 2 * pi * cemberYariCap;
  return cemberCevre;
}

console.log("Görev 1: Çemberin Çevresi:", CemberinCevresi(25));

/* (Oto test yok) Yukarıdaki CemberinCevresi fonksiyonunu yarıçap = 5 vererek aşağıda çalıştırıp, sonucu konsolda gözlemleyin (console.log)  */

console.log("Örnek Görev 2:", CemberinCevresi(5));
...
function CemberinAlani(cemberYariCap, a) {
  const cemberAlan = a * Math.pow(cemberYariCap, 2);
  return cemberAlan;
}

console.log("Görev 2: Çemberin Alanı:", CemberinAlani(25, pi));

/* (Oto test yok) Yukarıdaki CemberinAlani fonksiyonunu yarıçap = 15 vererek aşağıda çalıştırıp, sonucu konsolda gözlemleyin (console.log)  */
console.log("Örnek Görev 2: Çemberin Alanı:", CemberinAlani(25, pi));
/* 	GÖREV 3:
...
console.log("Örnek Görev 3: Dizideki sayı miktarı:", sayilar.length);
...
function buyuksayi(diziArr) {
  let enbuyuk = [];

  for (let i = 0; i < diziArr.length; i++) {
    if (diziArr[i] > enbuyuk) {
      enbuyuk = diziArr[i];
    }
  }
  return enbuyuk;
...
enbuyuk = buyuksayi(sayilar);

console.log("Görev 3a: En büyük sayi:", enbuyuk);

function kucuksayi(diziArr) {
  let enkucuk = diziArr[0];

  for (let i = 0; i < diziArr.length; i++) {
    if (diziArr[i] < enkucuk) {
      enkucuk = diziArr[i];
    }
  }
  return enkucuk;
...
enkucuk = kucuksayi(sayilar);

console.log("Görev 3a: En kücük sayi:", enkucuk);

// 3b çözümü:

function ucetambolunen(arrDizi) {
  let ucetambolunenler = [];
  arrDizi.forEach((item) => {
    if (item % 3 === 0) {
...
ucetambolunenler = ucetambolunen(sayilar);

console.log("Görev 3b: Üçe tam bolunenler", ucetambolunenler);
//3c çözümü:

function ucebolunenleritopla(ucArrDizi) {
  const ucebolunenlerintoplami = ucArrDizi.reduce(
...
ucebolunenlerintoplami = ucebolunenleritopla(ucetambolunenler);

console.log("Görev 3c: Üçe tam bolunenlerin toplami", ucebolunenlerintoplami);
//3d çözümü

// 3d. `besyuzdenkucuksayilar` adında bir dizi oluşturarak, sayilar dizisinin içindeki 500'den küçük sayıları bu diziye atayın (.filter metodunu kullanın)

function besYuzBulma(arrDizi) {
  const besyuzdenkucuksayilar = arrDizi.filter((item) => item < 500);
  return besyuzdenkucuksayilar;
}

besyuzdenkucuksayilar = besYuzBulma(sayilar);

console.log("Görev 3d: Beşyüzdenbüyüksayilar", besyuzdenkucuksayilar);
...
siralisayilar = siralama(besyuzdenkucuksayilar);
...
function tekrarEdenSayi(arrDizi) {
  ...
  arrDizi.forEach((item) => {
    if (tekrarlar[item] > 1) {
      let tekrarSayisi = 0;
      tekrarSayisi = tekrarlar[item];
      tekraredensayilar.push(
        `${item} sayısı ${tekrarSayisi} kere tekrar edilmiştir`
      );
    }
  });
...
tekraredensayilar = tekrarEdenSayi(sayilar);
console.log("Görev 3f: Tekrar eden sayilar", tekraredensayilar);
